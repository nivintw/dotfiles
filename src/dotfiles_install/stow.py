...
# then refuse to overwrite. The source of truth for tests/test_consistency.py.
#
# VS Code's settings.json is deliberately absent here: it's no longer a stowed file (see
# vscode_setup.py) — it's generated by its own phase the same way Claude's settings.json is,
# so there's nothing for stow to clear a path for.
...
PREK_SHIM_MARKER = "File generated by prek"
...
def _migrate_git_template_hooks(ctx: InstallContext) -> None:
    ...
    for hook in sorted(p for p in tmpl_hooks.iterdir() if not p.name.startswith(".")):
        if hook.is_symlink():
            continue
        if PREK_SHIM_MARKER in commands.read_text_or_empty(hook):
            hook.unlink()
