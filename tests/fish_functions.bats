...
# The other refused root: from `/` itself, forrepos must refuse — its repo set there is every
# repo on the machine. The guard fires before any fan-out, so cd'ing to / and running it is safe.
@test "forrepos refuses to fan out from /" {
  home="$(mktemp -d)"
  run env HOME="$home" fish -c "cd /; source '$FUNCDIR/forrepos.fish'; forrepos echo hi"
  rm -rf "$home"
  [ "$status" -eq 1 ]
  [[ "$output" == *"refusing to fan out"* ]]
}
