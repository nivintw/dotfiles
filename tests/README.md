...
**twice**, to prove idempotency), and gates on `dotfiles-install --verify-stream`'s `OK`/`BAD`
output — the one thing the unit/config suites can't do: prove the installer works on a genuinely
clean machine. It tolerates only the Touch-ID-no-sensor `BAD` (a VM has no biometric sensor); the
...
downloads a headless VM doesn't need; the verification runs core-aware (`--verify-stream --core`)
to match. The full-baseline install (casks included) is still what a real machine runs and what
